...
class Player(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.change_y != 1 and self.change_y != 0:
            ...
            if self.k != 40:
                ...
                www = True
                if self.k == 8:
                    shoot_sound = pygame.mixer.Sound('run.wav')
                    shoot_sound.set_volume(0.1)
                    shoot_sound.play()
                if self.k == 28:
                    ru_sound = pygame.mixer.Sound('run.wav')
                    ru_sound.set_volume(0.1)
                    ru_sound.play()
                if not self.right:
                    self.flip()
                self.rect = self.image.get_rect(topleft=(self.rect.x, self.rect.y))
    ...
    def reload(self):  # перезарядка выстрела
        global t1
        global time_delta
        global canshoot
    ...
    def reload_tp(self):  # перезарядка тп
        global t2
    ...
    def tele(self):  # отображение шарика с тп
        global cantp
        if cantp:
            global gg
            global t2
    ...
    def jump(self):
        ...
        if len(platform_hit_list) > 0 or self.rect.bottom >= SCREEN_HEIGHT:
            self.change_y = -16
            shoot_sound = pygame.mixer.Sound('jump.wav')
...
class Platform(pygame.sprite.Sprite):
    def __init__(self, width, height):
        ...
        self.image = pygame.image.load('объекты/platform.png')
...
class Bullet(pygame.sprite.Sprite):  # класс для стрел
...
class Button:  # класс для кнопок(планировалось использовать в меню но увы
    def __init__(self, width, height):
        self.widht = width
        self.height = height
    ...
    def draw(self, x, y, message, action=None, font_size=30):
        ...
        if x < mouse[0] < x + self.widht and y < mouse[1] < y + self.height:
            pygame.draw.rect(screen, self.active_clr, (x, y, self.widht, self.height))

            if click[0] == 1:
                pygame.time.delay(300)

        else:
            pygame.draw.rect(screen, self.inactive_clr, (x, y, self.widht, self.height))

        f1 = pygame.font.Font(None, font_size)
        text1 = f1.render(message, True,
                          (80, 80, 80))
...
def show_menu():  # меню
    pygame.init()
    menu_bckgr = pygame.image.load('ummer.jpg')
    ...
    while show:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1:
                    show = False
            start_btn.draw(300, 200, 'Start_game', None)
...
def main_l():
    ...
    while not done:
        ...
        if len(particles2) != 0 and not cantp:
            for particle in particles2:
                particles2.remove(particle)
        if ww:
            p = Player()
        ...
        if not gg and not cantp and schet != 0:
            if rr:
                for i in range(100):
                    particles3.append([[tx - random.randint(-25, 0) + 5, ty - random.randint(-50, 0)],
